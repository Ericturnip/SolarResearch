...
class CIMAdapter(ProductAdapter):
    """Adapter for PUNCH L3 CIM files.

    CIM is treated as a single-layer product. The adapter normalizes the FITS
    image into one 2D layer named ``brightness`` and leaves binning, S10
    conversion, filtering, and TXT writing to the shared v4 processing code.
    """

    product = "CIM"
    default_layer = "brightness"
    aliases = {"brightness", "cim", "data", "image", "unpolarized", "b"}
    ...
    def load_layer(self, path: str | Path, layer: str | None = None) -> LayerFrame:
        ...
        if requested.lower() not in self.aliases:
            raise ValueError(
                f"CIM has one layer named '{self.default_layer}'. "
        ...
        with fits.open(path, memmap=False) as hdul:
            ...
            if data.ndim != 2:
                raise ValueError(f"CIM science image must be 2D; got shape {data.shape} from {path}")
    ...
    def _find_science_image_hdu(hdul: fits.HDUList):
        ...
        raise ValueError(f"Could not find a 2D CIM science image HDU. HDUs seen: {shapes}")
    ...
    def _timestamp_from_headers(*headers) -> Time:
        ...
        raise ValueError("CIM file has no DATE-AVG/DATE-OBS/DATE-BEG/DATE-END timestamp.")
