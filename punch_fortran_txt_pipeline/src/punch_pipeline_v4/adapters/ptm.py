...
class PTMAdapter(ProductAdapter):
    """Adapter for PUNCH L3 PTM files.

    PTM is a 3-layer polarized mosaic datacube:
    - layer 0: Polar_B
    - layer 1: Polar_pB
    - layer 2: Polar_pBp

    For the current pipeline, we mainly want Polar_pB.
    """
    ...
    def load_layer(self, path: str | Path, layer: str | None = None) -> LayerFrame:
        ...
        with fits.open(path, memmap=False) as hdul:
            ...
            if cube.ndim != 3:
                raise ValueError(f"PTM science data must be 3D; got shape {cube.shape} from {path}")

            # Expected PTM shape is (3, Y, X)
            if cube.shape[0] == 3:
                data = cube[layer_i, :, :]
            elif cube.shape[-1] == 3:
                data = cube[:, :, layer_i]
                ...
                raise ValueError(
                    f"Cannot identify PTM layer axis. Expected one axis of length 3; got shape {cube.shape}"
                )

            primary_header = hdul[0].header if len(hdul) > 0 else header
            timestamp = self._timestamp_from_headers(header, primary_header)

            # Use celestial WCS to drop STOKES/layer axis.
    ...
    def _find_science_cube_hdu(hdul: fits.HDUList):
        ...
        raise ValueError(f"Could not find a 3D PTM science cube HDU. HDUs seen: {shapes}")
    ...
    def _timestamp_from_headers(*headers) -> Time:
        ...
        raise ValueError("PTM file has no DATE-AVG/DATE-OBS/DATE-BEG/DATE-END timestamp.")
